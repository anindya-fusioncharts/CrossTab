...
function Column(drawComponents, parsedJSON, index) {
    ...
    width = this.parsedJSON.chart.width;
    this.xAxisTickList = this.parsedJSON.TickList.xAxis;
...
Column.prototype.col = function(count) {
    ...
    var pointRightLimit = this.drawComponents.width + this.drawComponents.marginX;
    ...
    for (var k = 0; k < this.parsedJSON.data[this.index].length; k++) {
        ...
        if (this.parsedJSON.chart.xAxisType == 'date') {
            ...
            width = this.colPadding - this.colPadding / 4;
        }
        x = point.x - width / 2;
        if (this.parsedJSON.chart.xAxisType == 'date') {
